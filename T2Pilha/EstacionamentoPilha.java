...
public class EstacionamentoPilha {
    ...
    public EstacionamentoPilha (int capacidade) {
    ...
    public void push(CarroPilha carro) {
        if (estaCheio()) {
    ...
    public CarroPilha pop() {
        if (estaVazia()) {
            return null;
        }
        CarroPilha temp = topo.getInfo();
        topo = topo.getProximo();
        return temp;
    }

    public CarroPilha peek() {
        if (estaVazia()) {
            return null;
        }
        return topo.getInfo();
    ...
    public void sair(String placa) {
        ...
        if (!estaVazia() && peek().getPlaca().equals(placa)) {
            CarroPilha carro = pop();
            LocalDateTime horarioSaida = LocalDateTime.now();
            Duration permanencia = Duration.between(carro.getHorarioEntrada(), horarioSaida);
            System.out.println("Carro " + placa + " saiu às " + horarioSaida + ". Tempo de permanência foi: " + permanencia.toMinutes() + " minutos. Manobras: " + manobras + ".");
